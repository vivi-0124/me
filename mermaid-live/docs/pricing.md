...
同じワイヤ API を話す [OpenJev](openjev.md) に向き先を変えれば、TypeSafe への支払いは発生しない。

| | コスト | 必要なもの |
|---|---|---|
| TypeSafe の Jev | $0.042 / 1M 入力 | キー（早期アクセス、ウェイトリスト） |
| OpenJev のホスト版（Codiv） | 入力 100M トークンまで無料、カード不要 | サインアップ |
| OpenJev を自分の GPU で | 電気代だけ | NVIDIA GPU 24GB 以上、重み 18GB |
| 何も設定しない | ゼロ | なし（規則ベースで動く） |

100M トークンの無料枠は、この Mod の実測だと **おおよそ 13,000〜60,000 応答ぶん**。
